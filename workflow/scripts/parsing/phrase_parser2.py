...
class PhraseParser(Parser):
    ...
    @_('ART ORDN', 'ART PRDE', 'ART QUANT', 
       'ART CARD1', 'ART ADJV', 
       'ART PREP', # covers some borderline preps 
      )
    def defi(self, p):
    ...
    # -- parallel phrases --
    @_('np CONJ np', 'defi CONJ defi', 'pp CONJ pp',
       'appo CONJ appo', 'adjv CONJ adjv', 'demon CONJ demon',
       'quant CONJ quant', 'gp CONJ gp', 'para CONJ para',
       'np CONJ para', 'defi CONJ para', 'num CONJ num',
       'pp CONJ para', 'appo CONJ para', 'adjv CONJ para', 
       'demon CONJ para', 'quant CONJ para', 'gp CONJ para') 
    def para(self, p):
        conj = [p[1].slot, p[2], 'CONJ']
        return [p[0], conj, 'PARA']
